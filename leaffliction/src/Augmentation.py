...
def skew_image(image):
    ...
    return image.transform((new_width, height), Image.AFFINE, (1, skew_factor, -width_shift if skew_factor > 0 else 0, 0, 1, 0))


# The matrix is represented as a tuple (a, b, c, d, e, f). For horizontal shearing, set b and e to the shear factor
def shear_image(image):
    shear_factor=0.2
    return image.transform(image.size, Image.AFFINE, (1, shear_factor, 0, shear_factor, 1, 0))
...
def balance(source, dest):
    ...
    for index, label in enumerate(labels):
        ...
        while count < max_val:
            ...
            count += 6
        print(f"{index} val is NOW {count}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('path', help="the image path or the base directory path if balanced")
    parser.add_argument('--dest', help="the images destination", nargs='?', default="")
    parser.add_argument('--balance', action=argparse.BooleanOptionalAction)

    args = parser.parse_args()
    if args.balance :
        assert args.dest != "", "You must specify a destination directory to create a balanced dataset"
        assert (os.path.exists(args.path)), f"Couln't find the {args.path} directory"
        balance(args.path, args.dest)
    else:
        assert (os.path.exists(args.path)), f"Couln't find the image at\
{args.path}"
